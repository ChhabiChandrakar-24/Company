...
from chhabi.chhabi_apps import TWO_FACTORS_AUTHENTICATION
...
CACHE_KEY = "chhabi_company_models_cache_key"
...
class CompanyMiddleware:
    ...
    def _add_company_filter(self, model, company_id):
        ...
        is_chhabi_manager = isinstance(model.objects, ChhabiCompanyManager)
        ...
        if is_company_model:
            ...
            elif is_chhabi_manager and related_company_field:
                model.add_to_class(
            ...
            for app_label, models in app_model_mappings.items():
                if apps.is_installed(app_label):
                    company_models.extend(
                        [get_chhabi_model_class(app_label, model) for model in models]
                    )
    ...
    def __call__(self, request):
        if getattr(request, "user", False) and not request.user.is_anonymous:
            # Bypass company filters and session setup for client portal users
            if hasattr(request.user, "crm_client_profile"):
