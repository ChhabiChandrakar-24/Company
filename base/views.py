...
from chhabi import chhabi_apps
...
from chhabi.chhabi_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from chhabi.methods import get_chhabi_model_class, remove_dynamic_url
from chhabi_audit.forms import HistoryTrackingFieldsForm
from chhabi_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/chhabi_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/chhabi_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/chhabi_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/chhabi_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/chhabi_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/chhabi_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/chhabi_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/chhabi_job_position.html",
...
def login_user(request):
    ...
    if request.method == "POST":
        ...
        # `/` is the public company website. A normal Horilla login should
        # return to the original internal dashboard unless a protected page
        # supplied an explicit, safe `next` URL.
        next_url = request.GET.get("next") or reverse("home-page")
        query_params = request.GET.dict()
        query_params.pop("next", None)
        params = urlencode(query_params)
        # If `next_url` is already a concrete path (e.g., "/subscription/plans/"),
        # resolve it to confirm; otherwise treat it as a named URL.
        try:
            resolve(next_url)
        except Resolver404:
            try:
...
class ChhabiPasswordResetView(PasswordResetView):
    """
    Chhabi View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    Chhabi View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not chhabi_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_chhabi_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from Chhabi")
...
@login_required
@hx_request_required
@permission_required("base.change_chhabimailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = ChhabiMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_chhabimailtemplate")
...
@login_required
@permission_required("base.delete_chhabimailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = ChhabiMailTemplate.objects.filter(id__in=ids).delete()
...
@login_required
@permission_required("base.delete_company")
@require_http_methods(["POST"])
def company_delete(request, obj_id):
    """Delete a company only when the user has the explicit company permission."""
    return object_delete(
        request,
        obj_id,
        model=Company,
        redirect_path="/settings/company-view/",
    )
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_chhabi_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_chhabi_model_class(
...
@login_required
@permission_required("chhabi_audit.change_accountblockunblock")
...
@login_required
@permission_required("chhabi_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("chhabi_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("chhabi_audit.change_audittag")
...
@login_required
def get_chhabi_installed_apps(request):
