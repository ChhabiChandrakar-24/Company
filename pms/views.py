...
from chhabi_automations.methods.methods import generate_choices
from chhabi_automations.methods.serialize import serialize_form
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if feedback.based_on == "employee":
                try:
                    notify.send(
                        User.objects.filter(username="Chhabi Bot").first(),
...
def view_meetings(request):
    ...
    if getattr(request, "organization", None):
        meetings = meetings.filter(organization=request.organization)
    ...
    if getattr(request, "organization", None):
        all_meetings = all_meetings.filter(organization=request.organization)
...
def create_meetings(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            # If creating a new meeting, consume rooms quota
            if not instance:
                from pms.entitlement import EntitlementError
                try:
                    request.entitlement.consume_quota('rooms_created_today')
                except EntitlementError as exc:
                    messages.error(request, f"Quota Exceeded: {exc.detail}")
...
# ============================================================================
# SaaS Multi-Organization & Metrics Dashboard views
# ============================================================================
from django.db.models import Sum
from django.core.exceptions import PermissionDenied
from pms.models import Organization, OrganizationMember, MeetingPlan, UserSubscription

@login_required
def saas_dashboard_view(request):
    """
    Authorized metrics dashboard for multi-organization SaaS tracking.
    """
    if request.user.is_superuser:
        # Superuser sees global aggregate metrics across all organizations
        orgs = Organization.objects.all()
        total_orgs = orgs.count()
        total_active_subs = UserSubscription.objects.filter(status="active").count()
        
        # Meetings count (superusers bypass TenantChhabiCompanyManager filter)
        total_meetings = Meetings.objects.count()
        total_users = User.objects.count()
        
        # API usage & storage totals
        api_totals = UserSubscription.objects.filter(status="active").aggregate(
            total_api=Sum("api_calls_today"),
            total_storage=Sum("storage_used_mb")
        )
        total_api_calls = api_totals.get("total_api") or 0
        total_storage_mb = api_totals.get("total_storage") or 0
        
        # Payment transaction states
        from payments.models import PaymentTransaction
        txns = PaymentTransaction.objects.all().order_by("-created_at")[:20]
        ...
        # Regular user sees metrics scoped to their active organization membership
        membership = request.user.organization_memberships.select_related("organization").first()
        if not membership:
            # If they don't have an organization, redirect them to create one
            return redirect("saas-create-org")

        org = membership.organization
        sub = UserSubscription.objects.filter(organization=org, status="active").select_related("plan").first()
        
        # Scoped meetings count (TenantChhabiCompanyManager automatically filters by org)
        ...
        api_calls_today = sub.api_calls_today if sub else 0
        api_limit = sub.api_calls_limit if sub else 0
        rooms_created_today = sub.rooms_created_today if sub else 0
        rooms_limit = sub.rooms_limit if sub else 0
        storage_used_mb = sub.storage_used_mb if sub else 0
        storage_limit_mb = sub.storage_limit_mb if sub else 0
        
        # Scoped transactions
        from payments.models import PaymentTransaction
        txns = PaymentTransaction.objects.filter(user_subscription__organization=org).order_by("-created_at")
...
@login_required
def saas_create_org_view(request):
    """
    View to create a new SaaS organization and register owner.
    """
    if request.method == "POST":
        ...
        if not org_name:
            ...
            if Organization.objects.filter(name__iexact=org_name).exists():
                ...
                slug = django.utils.text.slugify(org_name)
                ...
                # Automatically create a default free subscription for the organization
                free_plan = MeetingPlan.objects.filter(code="free").first()
                if free_plan:
                    ...
                    UserSubscription.objects.create(
                        ...
                        organization=org,
                        plan=free_plan,
                        api_calls_limit=free_plan.max_api_calls_per_day,
                        rooms_limit=free_plan.max_concurrent_rooms,
    ...
    return render(request, "dashboard/saas_create_org.html")


@login_required
def saas_add_member_view(request):
    """
    View to add/invite an employee/user to the active organization.
    Only accessible by owner or admin roles.
    """
    membership = request.user.organization_memberships.select_related("organization").first()
    if not membership or membership.role not in ["owner", "admin"]:
        raise PermissionDenied("Only organization owners and admins can invite members.")
    ...
    if request.method == "POST":
        ...
        if OrganizationMember.objects.filter(organization=org, user=target_user).exists():
            ...
            OrganizationMember.objects.create(organization=org, user=target_user, role=role)
            messages.success(request, f"User '{username}' successfully added to the organization as {role}.")

    return redirect("saas-members")


@login_required
def saas_members_view(request):
    """
    View to list members of the active organization.
    """
    membership = request.user.organization_memberships.select_related("organization").first()
    if not membership:
        return redirect("saas-create-org")
    ...
    # Available users to invite (exclude existing members)
    existing_user_ids = members.values_list("user_id", flat=True)
    inviteable_users = User.objects.exclude(id__in=existing_user_ids).order_by("username")
    ...
    return render(request, "dashboard/saas_members.html", context)


@login_required
def saas_change_subscription_view(request):
    """
    View to change the plan of the organization. Only accessible by owner role.
    """
    membership = request.user.organization_memberships.select_related("organization").first()
    if not membership or membership.role != "owner":
        raise PermissionDenied("Only organization owners can change subscription plans.")

    org = membership.organization
    plans = MeetingPlan.objects.filter(is_active=True)
    ...
    if request.method == "POST":
        ...
        UserSubscription.objects.filter(organization=org, status="active").update(status="expired")
        
        # Create new subscription
        status = "active" if plan.price_inr == 0 else "pending"
        from datetime import timedelta
        ...
        if status == "active":
            messages.success(request, f"Plan updated to '{plan.name}' successfully!")
            return redirect("saas-dashboard")
        else:
            messages.warning(request, f"Plan selected: '{plan.name}'. Please complete the payment to activate.")
            return redirect("saas-dashboard")
    ...
    return render(request, "dashboard/saas_change_plan.html", context)
