...
from chhabi.chhabi_settings import BIO_DEVICE_THREADS
from chhabi.settings import TIME_ZONE
...
@install_required
@permission_required("biometric.delete_biometricemployees")
def delete_chhabi_cosec_user(request, user_id, device_id):
    """
...
def map_biometric_users(request, device_id):
    """
    Maps an chhabi employee to a biometric user on a specified biometric device.
    """
    device = BiometricDevices.find(device_id)
...
def dahua_biometric_attendance_logs(device):
    """
    Retrieves logs from a Dahua biometric device and marks attendance in Chhabi.

    This function fetches biometric logs from the specified device, processes the attendance records,
    and updates the attendance system in Chhabi. If an employee has an active clock-in record,
