"""
chhabi_automations/views/cbvs.py
"""
...
from base.models import ChhabiMailTemplate
from chhabi.decorators import login_required, permission_required
from chhabi_automations import models
from chhabi_automations.filters import AutomationFilter
from chhabi_automations.forms import AutomationForm
from chhabi_views.generic.cbv import views


@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("chhabi_automations.view_mailautomation"), name="dispatch"
)
class AutomationSectionView(views.ChhabiSectionView):
    """
    ...
    template_name = "chhabi_automations/section_view.html"
...
class AutomationNavView(views.ChhabiNavView):
    """
    AutomationNavView
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.actions = []
        if self.request.user.has_perm("chhabi_automations.add_mailautomation"):
            self.create_attrs = f"""
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("chhabi_automations.change_mailautomation"), name="dispatch"
)
class AutomationFormView(views.ChhabiFormView):
    """
    AutomationFormView
    """
    ...
    template_name = "chhabi_automations/automation_form.html"
...
class AutomationListView(views.ChhabiListView):
    """
    AutomationListView
    """
...
class AutomationDetailedView(views.ChhabiDetailedView):
    """
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("chhabi_automations.add_mailautomation"), name="dispatch"
)
class LoadAutomationsView(View):
    template_name = "chhabi_automations/load_automation.html"
    ...
    def post(self, request):
        ...
        for template_json in templates_raw:
            if template_json["pk"] in required_template_pks:
                ...
                existing = ChhabiMailTemplate.objects.filter(
        ...
        for automation_json in selected_automations:
            ...
            mail_template = ChhabiMailTemplate.objects.filter(
                body=template_body
            ).first()
