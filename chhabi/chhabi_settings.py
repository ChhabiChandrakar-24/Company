...
from chhabi.chhabi_apps import INSTALLED_APPS
...
if settings.env("AWS_STORAGE_BUCKET_NAME", default=None):
    # Access keys are optional. In AWS production, attach an IAM role to the
    # EC2 instance and boto3 will obtain short-lived credentials automatically.
    AWS_ACCESS_KEY_ID = settings.env("AWS_ACCESS_KEY_ID", default=None)
    AWS_SECRET_ACCESS_KEY = settings.env("AWS_SECRET_ACCESS_KEY", default=None)
...
if settings.env("AWS_STORAGE_BUCKET_NAME", default=None) and "storages" in INSTALLED_APPS:
...
DEFAULT_LDAP_CONFIG = {
    ...
    "BIND_DN": settings.env("BIND_DN", default="cn=admin,dc=chhabi,dc=com"),
    "BIND_PASSWORD": settings.env("BIND_PASSWORD", default="chhabi"),
    "BASE_DN": settings.env("BASE_DN", default="ou=users,dc=chhabi,dc=com"),
}
...
def load_ldap_settings():
    ...
    try:
        ...
        from chhabi_ldap.models import LDAPSettings
