...
from chhabi.group_by import group_by_queryset as group_by
...
def update_status(request):
    ...
    contract_notice_end_date = (
        get_chhabi_model_class(app_label="payroll", model="contract")
...
def get_notice_period(request):
    ...
    employee_id = request.GET["employee_id"]
    if apps.is_installed("payroll"):
        Contract = get_chhabi_model_class(app_label="payroll", model="contract")
        employee_contract = (
            (
...
def offboarding_dashboard(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_chhabi_model_class("recruitment", "candidate")
...
if apps.is_installed("asset"):
    ...
    def dashboard_asset_table(request):
        ...
        """
        AssetAssignment = get_chhabi_model_class(
...
if apps.is_installed("pms"):
    ...
    def dashboard_feedback_table(request):
        ...
        Feedback = get_chhabi_model_class(app_label="pms", model="feedback")
...
# ---------------------------------------------------------------------------
# Termination / Offboarding Workflow Views
# ---------------------------------------------------------------------------
...
class TerminationWorkflowForm(forms.ModelForm):
    class Meta:
        model = TerminationWorkflow
        fields = ["employee", "reason", "effective_date", "termination_letter"]
        widgets = {
            "employee": forms.Select(attrs={"class": "form-control"}),
            "reason": forms.Textarea(attrs={"class": "form-control", "rows": 4}),
            "effective_date": forms.DateInput(attrs={"class": "form-control", "type": "date"}),
            "termination_letter": forms.Textarea(attrs={"class": "form-control", "rows": 6}),
        }


class TerminationChecklistForm(forms.ModelForm):
    class Meta:
        model = TerminationWorkflow
        fields = ["assets_returned", "access_revoked"]


@login_required
def termination_list(request):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    terminations = TerminationWorkflow.objects.all()
    return render(request, "termination/list.html", {"terminations": terminations})


@login_required
def termination_initiate(request):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    form = TerminationWorkflowForm(request.POST or None)
    if request.method == "POST" and form.is_valid():
        termination = form.save(commit=False)
        termination.initiated_by = request.user
        termination.status = "initiated"
        termination.save()
        messages.success(request, _("Termination workflow initiated successfully."))
        return redirect("termination-list")
    return render(request, "termination/initiate.html", {"form": form})


@login_required
def termination_detail(request, termination_id):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    termination = get_object_or_404(TerminationWorkflow, id=termination_id)
    checklist_form = TerminationChecklistForm(instance=termination)
    return render(request, "termination/detail.html", {
...
@login_required
def termination_approve(request, termination_id):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    termination = get_object_or_404(TerminationWorkflow, id=termination_id)
    termination.status = "approved"
    termination.save()
    messages.success(request, _("Termination request approved."))
    return redirect("termination-detail", termination_id=termination.id)


@login_required
def termination_reject(request, termination_id):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    termination = get_object_or_404(TerminationWorkflow, id=termination_id)
    termination.status = "rejected"
    termination.save()
    messages.success(request, _("Termination request rejected."))
    return redirect("termination-detail", termination_id=termination.id)


@login_required
def termination_update_checklist(request, termination_id):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    termination = get_object_or_404(TerminationWorkflow, id=termination_id)
    form = TerminationChecklistForm(request.POST or None, instance=termination)
    if request.method == "POST" and form.is_valid():
        form.save()
        messages.success(request, _("Clearance checklist updated successfully."))
    return redirect("termination-detail", termination_id=termination.id)


@login_required
def termination_complete(request, termination_id):
    if not request.user.has_perm("offboarding.change_offboarding") and not request.user.is_superuser:
        raise PermissionDenied
    termination = get_object_or_404(TerminationWorkflow, id=termination_id)
    if termination.status != "approved":
        messages.error(request, _("Termination workflow must be approved before completion."))
        return redirect("termination-detail", termination_id=termination.id)
    if not termination.assets_returned or not termination.access_revoked:
        messages.error(request, _("All clearance checklist items must be completed before offboarding finalization."))
        return redirect("termination-detail", termination_id=termination.id)
        
    termination.status = "completed"
    termination.save()
    
    # Final status update: deactivate the employee, preserving their historic records
    employee = termination.employee
    employee.is_active = False
    employee.save()
    
    messages.success(request, _("Offboarding completed successfully. Employee has been set to inactive."))
    return redirect("termination-list")
