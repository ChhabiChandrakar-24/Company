...
from chhabi.models import ChhabiModel, upload_path
...
class Project(ChhabiModel):
    ...
    def save(self, *args, **kwargs):
        is_new, request = self.pk is None, getattr(
            chhabi_middlewares._thread_locals, "request", None
        )
...
class ProjectStage(ChhabiModel):
    """
    ProjectStage model
    """
    ...
    objects = ChhabiCompanyManager("project__company_id")
...
class TimeSheet(ChhabiModel):
    ...
    objects = ChhabiCompanyManager("project_id__company_id")
...
class ProjectClosure(models.Model):
    """Tracks the closure details of a Project."""
    project = models.OneToOneField(Project, on_delete=models.CASCADE, related_name="closure")
    delivered_modules = models.JSONField(default=dict, help_text=_("Modules/Features delivered"))
    delivery_status = models.CharField(max_length=50, default="pending", help_text=_("Delivery verification status"))
    payment_status = models.CharField(max_length=50, default="pending", help_text=_("Payment verification status"))
    approval_status = models.CharField(max_length=50, default="pending", help_text=_("Internal approvals status"))
    closed_at = models.DateTimeField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Closure for {self.project.title}" 

    class Meta:
        verbose_name = _("Project Closure")
        verbose_name_plural = _("Project Closures")

class ProjectCommunicationLog(models.Model):
    """Log of completion communications sent to the client."""
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name="communication_logs")
    channel = models.CharField(max_length=50, help_text=_("Channel used (email, slack, etc.)"))
    summary_hash = models.CharField(max_length=64, help_text=_("Hash of the message content to dedupe"))
    sent_at = models.DateTimeField(auto_now_add=True)
    status = models.CharField(max_length=20, default="sent", help_text=_("Send status"))

    def __str__(self):
        return f"{self.channel} log for {self.project.title} at {self.sent_at}" 

    class Meta:
        verbose_name = _("Project Communication Log")
        verbose_name_plural = _("Project Communication Logs")
