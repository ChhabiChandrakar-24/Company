...
from chhabi.models import ChhabiModel, has_xss, upload_path
...
def send_employee_welcome_email(employee_name, employee_email, temporary_password):
    """Send initial login credentials once for a newly created employee."""
    ...
    html_body = f"""
        ...
        <p><strong>Login ID:</strong> {employee_email}<br>
        <strong>Temporary password:</strong> {temporary_password}</p>
        <p>For security, please change this password immediately after your first login.</p>
    """
    try:
        message = EmailMultiAlternatives(subject, plain_body, to=[employee_email])
        message.attach_alternative(html_body, "text/html")
        message.send(fail_silently=False)
    except Exception:
        # Account creation must not fail just because the mail server is unavailable.
        logger.exception("Could not send welcome email to employee %s", employee_email)
...
class Employee(models.Model):
    ...
    def get_archive_condition(self):
        ...
        if apps.is_installed("onboarding"):
            OnboardingStage = get_chhabi_model_class("onboarding", "onboardingstage")
            OnboardingTask = get_chhabi_model_class("onboarding", "onboardingtask")
        ...
        if apps.is_installed("recruitment"):
            Recruitment = get_chhabi_model_class("recruitment", "recruitment")
            Stage = get_chhabi_model_class("recruitment", "stage")
    ...
    def check_online(self):
        ...
        """
        if apps.is_installed("attendance"):
            Attendance = get_chhabi_model_class("attendance", "attendance")
        ...
        if employee.employee_user_id is None:
            ...
            if prev_employee is None:
                transaction.on_commit(
                    lambda: send_employee_welcome_email(
                        self.get_full_name(), username, password
                    )
                )
            # default permissions
...
class EmployeeWorkInformation(models.Model):
    ...
    probation_status = models.CharField(
        ...
        choices=(
            ("none", _("None")),
            ("on_probation", _("On Probation")),
            ("confirmed", _("Confirmed")),
            ("extended", _("Extended")),
        ),
        default="none",
        verbose_name=_("Probation Status")
    )
    history = ChhabiAuditLog(
...
class EmployeeBankDetails(ChhabiModel):
    """
...
class EmployeeNote(ChhabiModel):
    """
    EmployeeNote model
    """
...
class PolicyMultipleFile(ChhabiModel):
    """
...
class DisciplinaryAction(ChhabiModel):
    """
    Disciplinary model
    """
...
class EmployeeGeneralSetting(ChhabiModel):
    """
...
class ProfileEditFeature(ChhabiModel):
    """
    ProfileEditFeature
    """
...
class JoiningLetterTemplate(ChhabiModel):
...
class IssuedJoiningLetter(ChhabiModel):
    STATUS_CHOICES = (
        ("draft", "Draft"),
        ("issued", "Issued"),
        ("accepted", "Accepted"),
    ...
    employee = models.ForeignKey(Employee, on_delete=models.CASCADE, related_name="issued_joining_letters")
    template = models.ForeignKey(JoiningLetterTemplate, on_delete=models.SET_NULL, null=True, blank=True)
    title = models.CharField(max_length=200)
    body = models.TextField(blank=True, null=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="draft")
    issued_date = models.DateField(null=True, blank=True)
    issued_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name="issued_joining_letters_issued")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.title} for {self.employee.get_full_name()}"
