...
from chhabi.filters import ChhabiPaginator
from chhabi.group_by import group_by_queryset
from chhabi.chhabi_settings import CHHABI_DATE_FORMATS
from chhabi.methods import get_chhabi_model_class
from chhabi_audit.models import AccountBlockUnblock, HistoryTrackingFields
from chhabi_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
@login_required
@manager_can_enter("chhabi_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="chhabi_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("chhabi_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "chhabi_documents.add_documentrequest")
    if request.method == "POST":
        form = DocumentRequestForm(request.POST)
        form = choosesubordinates(
            request, form, "chhabi_documents.add_documentrequest"
        )
        if form.is_valid():
            form = form.save()
...
@login_required
@hx_request_required
@manager_can_enter("chhabi_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("chhabi_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("chhabi_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("chhabi_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("chhabi_documents.add_document")
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = ChhabiPaginator(qryset, get_pagination())
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_chhabi_model_class(
                    ...
                    OnboardingStage = get_chhabi_model_class(
                    ...
                    OnboardingTask = get_chhabi_model_class(
        ...
        for employee_dict in employee_dicts:
            try:
                ...
                if existing_user is None:
                    ...
                    employee.save()
                    send_employee_welcome_email(
                        employee.get_full_name(), email, str(phone).strip()
                    )
            except Exception:
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        CHHABI_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_chhabi_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_chhabi_model_class(
...
def redeem_points(request, emp_id):
    ...
    if apps.is_installed("payroll"):
        EncashmentGeneralSettings = get_chhabi_model_class(
...
# ---------------------------------------------------------------------------
# Joining Letters Management Views
# ---------------------------------------------------------------------------
from django import forms
from employee.models import JoiningLetterTemplate, IssuedJoiningLetter, Employee
from django.core.exceptions import PermissionDenied

class JoiningLetterTemplateForm(forms.ModelForm):
    class Meta:
        model = JoiningLetterTemplate
...
class IssuedJoiningLetterForm(forms.ModelForm):
    class Meta:
        model = IssuedJoiningLetter
        fields = ["employee", "template", "title", "body", "status", "issued_date"]
        widgets = {
...
def render_letter_body(template_body, employee):
    work_info = getattr(employee, 'employee_work_info', None)
    designation = work_info.job_position_id.job_position if work_info and work_info.job_position_id else ""
    department = work_info.department_id.department if work_info and work_info.department_id else ""
    salary = str(work_info.basic_salary) if work_info else ""
    joining_date = work_info.date_joining.strftime("%d %b, %Y") if work_info and work_info.date_joining else ""
    company_name = work_info.company_id.company if work_info and work_info.company_id else ""
...
@login_required
def joining_letter_template_list(request):
    if not request.user.has_perm("employee.change_employee") and not request.user.is_superuser:
        raise PermissionDenied
    templates = JoiningLetterTemplate.objects.all()
    return render(request, "joining_letters/template_list.html", {"templates": templates})


@login_required
def joining_letter_template_create(request):
    if not request.user.has_perm("employee.change_employee") and not request.user.is_superuser:
        raise PermissionDenied
    form = JoiningLetterTemplateForm(request.POST or None)
    if request.method == "POST" and form.is_valid():
        form.save()
        messages.success(request, _("Joining letter template created successfully."))
        return redirect("joining-letter-template-list")
    return render(request, "joining_letters/template_form.html", {"form": form, "title": _("Create Template")})


@login_required
def joining_letter_template_edit(request, template_id):
    if not request.user.has_perm("employee.change_employee") and not request.user.is_superuser:
        raise PermissionDenied
    template_obj = get_object_or_404(JoiningLetterTemplate, id=template_id)
    form = JoiningLetterTemplateForm(request.POST or None, instance=template_obj)
    if request.method == "POST" and form.is_valid():
        form.save()
        messages.success(request, _("Joining letter template updated successfully."))
        return redirect("joining-letter-template-list")
    return render(request, "joining_letters/template_form.html", {"form": form, "title": _("Edit Template")})


@login_required
def joining_letter_template_delete(request, template_id):
    if not request.user.has_perm("employee.change_employee") and not request.user.is_superuser:
        raise PermissionDenied
    template_obj = get_object_or_404(JoiningLetterTemplate, id=template_id)
    template_obj.delete()
...
@login_required
def issued_joining_letter_list(request):
    # Regular employees can only see their own letters
    if not request.user.has_perm("employee.change_employee") and not request.user.is_superuser:
        employee = getattr(request.user, "employee_get", None)
        letters = IssuedJoiningLetter.objects.filter(employee=employee)
    else:
        letters = IssuedJoiningLetter.objects.all()
    return render(request, "joining_letters/issued_list.html", {"letters": letters})
...
def preview_joining_letter(request):
    ...
    rendered = render_letter_body(template_obj.body, employee)
    return JsonResponse({"body": rendered})


@login_required
def issue_joining_letter(request):
    if not request.user.has_perm("employee.change_employee") and not request.user.is_superuser:
        raise PermissionDenied
    form = IssuedJoiningLetterForm(request.POST or None)
    if request.method == "POST" and form.is_valid():
        letter = form.save(commit=False)
        letter.issued_by = request.user
        # Auto render if body is empty or we want to overwrite
        if not letter.body and letter.template:
            letter.body = render_letter_body(letter.template.body, letter.employee)
        letter.save()
        
        # If status is "issued", automatically store as an Employee document
        if letter.status == "issued":
            from chhabi_documents.models import Document
            from django.core.files.base import ContentFile
            doc = Document.objects.create(
                title=letter.title,
                employee_id=letter.employee,
                status="approved",
            )
            doc.document.save(f"joining_letter_{letter.id}.txt", ContentFile(letter.body))
            doc.save()

        messages.success(request, _("Joining letter processed successfully."))
        return redirect("issued-joining-letter-list")
    return render(request, "joining_letters/issue_form.html", {"form": form})


@login_required
def issued_joining_letter_action(request, letter_id, action_type):
    letter = get_object_or_404(IssuedJoiningLetter, id=letter_id)
    # Check permissions: only the employee themselves or HR can sign/update action
    is_owner = getattr(request.user, "employee_get", None) == letter.employee
    is_hr = request.user.has_perm("employee.change_employee") or request.user.is_superuser
    ...
    if action_type == "accept":
        ...
        letter.save()
        messages.success(request, _("Joining letter accepted."))
    elif action_type == "reject":
        letter.status = "rejected"
        letter.save()
        messages.success(request, _("Joining letter rejected."))
